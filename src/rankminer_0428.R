...
###---------------------------
#Deleting Columns not needed for further computations


agent$payroll_id <- NULL 
agent$hire_date <- NULL
agent$term_date <- NULL
agent$group <- NULL
agent$term_code <- NULL
agent$term_reason <- NULL
agent$work_shift <- NULL
...
agent$productivity <- NA
agent$productivity <- rowMeans(agent[,c("Jul_hrs_worked","Aug_hrs_worked","Sep_hrs_worked","Oct_hrs_worked","Nov_hrs_worked","Dec_hrs_worked")],na.rm = TRUE)
agent$productivity <- agent$productivity/agent$nom

#rev_generated
class(agent$Oct_revenue_generated)
